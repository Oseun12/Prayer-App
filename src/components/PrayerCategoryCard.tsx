  ...
  return (
    <div>
      ...
      <div className="mt-4 space-y-3">
        {category.prayers?.slice(0, 3).map(prayer => (
          <div key={prayer.id} className="p-4 bg-white/5 rounded-lg relative group">
            <p className="text-white/90">{prayer.text}</p>
            {prayer.verse && (
              <p className="text-sm mt-1 text-white/60 italic">{prayer.verse}</p>
            )}
